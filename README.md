...
- **Fused multi-engine search** — Bing (keyless) + Brave HTML (keyless) + Tavily + Exa + Brave in parallel; keyless mode always keeps two independent free channels (Bing + Brave HTML) for cross-checking, deduplicated by URL, cross-ranked by engine agreement and domain quality. A result found by 2+ independent engines is high-confidence; single-engine noise is demoted.
