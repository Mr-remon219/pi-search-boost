...
- **Fused multi-engine search** — Bing (keyless) + Tavily + Exa + Brave in parallel, deduplicated by URL, cross-ranked by engine agreement and domain quality. A result found by 2+ independent engines is high-confidence; single-engine noise is demoted.
...
- **Proactive-search policy** — a `<search_balance>` ruleset injected before agent start: when to search by default (facts, versions, time-sensitive, comparisons, unfamiliar topics), when to skip (local code, pure writing, fundamental stable concepts), when to stop (evidence sufficient, same query twice = loop, ~3 rounds diminishing returns), and autonomy rules (refine queries, curl fallback, direct URL fetching, prompt-injection guard).
